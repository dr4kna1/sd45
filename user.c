              ...
              switch (led_state)
                {
                    case 1  :    
                        ...
                        {
                            prcd_led1();
                            nstr = (conv_arr[Real_var]&0x0F00)>>8;
                            PORTD = decode_str(nstr); break;                                          // lit '1'
                        }
                        else
                            break;
                    case 2  :
                        if (service_info)
                        {
                            prcd_led2(); PORTD = decode_str((conv_arr[Real_var]&0x00F0)>>4) | 0x10; break;    // suppress dot 0x10
                        }
                        else
                            break;
                    case 3  :
                        if (service_info)
                        {
                            prcd_led3(); PORTD = decode_str(conv_arr[Real_var]&0x000F) | 0x10; break;
                        }
                        else
                            break;
                    case 4  :
